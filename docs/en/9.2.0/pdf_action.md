...
- `adjacent_type` (string) __Neighbours__ - How neighbouring tag pairs are found. Empty (Direct neighbours): next tag in the structure tree after Skip tags wrappers are ignored — use with regex Tags. A tag type: consecutive tags of that type in reading order — use with the matching Tags template preset. Skip tags always apply: matching wrappers (NonStruct, Div, …) are stepped over as if they were not in the tree.
