...
__Accessibility__: [Fix Role Mapping](#fix-role-mapping), [AutoTag](#autotag), [Clear Document Structure](#clear-document-structure), [Fix ID Tree](#fix-id-tree), [Fix Parent Tree](#fix-parent-tree), [Fix Invalid Elements](#fix-invalid-elements), [Fix Spaces](#fix-spaces), [Fix Headings](#fix-headings)

</div>

<div id="md-toc-annotations">

__Annotations__: [Fix Media Clip](#fix-media-clip), [Set Tab Order](#set-tab-order), [Fix Fonts for Form Filling](#fix-fonts-for-form-filling), [Tag Annotations](#tag-annotations), [Set Annotation Contents](#set-annotation-contents), [Set Annotation Flag](#set-annotation-flag), [Remove Annotation Properties](#remove-annotation-properties), [Flatten Annotations](#flatten-annotations), [Create Web Links](#create-web-links), [Delete Annotations](#delete-annotations)

</div>

<div id="md-toc-bookmarks">

__Bookmarks__: [Delete Bookmarks](#delete-bookmarks), [Create Bookmarks](#create-bookmarks)

</div>

<div id="md-toc-content">

__Content__: [Set Content Language](#set-content-language), [Delete Content](#delete-content), [Artifact Content](#artifact-content), [Flatten Form XObjects](#flatten-form-xobjects), [Clone Form XObjects](#clone-form-xobjects), [Fix Content Marks](#fix-content-marks), [Set Content Color](#set-content-color), [Split Content](#split-content)
...
__Fonts__: [Fix Fonts](#fix-fonts), [Set Charcode Unicode Mapping](#set-charcode-unicode-mapping)
...
- `template` (template) __Template__ - Layout template that controls how content is recognized and tagged. Load from a file or edit the template JSON inline. Leave empty to use the default template.

- `preflight` (bool) __Preflight__ - Analyze pages and apply the results to the default template before tagging. Ignored if a template is set
...
- `overwrite` (bool) __Overwrite__ - Replace existing tag structure if it already exist
...
`fix_fonts_for_form_filling`

Replace custom or embedded form fonts with standard fonts for reliable filling and display.

#### example:
```
{
    "name": "fix_fonts_for_form_filling"
}
```
### Tag Annotations
...
- `flag_mode` (int) __Flag Mode__ - Choose how the selected flags are applied to each annotation

  - __0__  - Turn on the selected flags
  - __1__  - Turn off the selected flags
  - __2__  - Set the F key to exactly the selected flags


- `flags` (flag) __Annotation Flags__ - Select annotation flags to set, clear, or replace (PDF Table 165)

  - __1__  - Do not display unknown annotation types without a handler
  - __2__  - Do not display, print, or allow user interaction
  - __4__  - Include the annotation when the page is printed
  - __8__  - Do not scale the annotation appearance with page magnification
  - __16__  - Do not rotate the annotation appearance with page rotation
  - __32__  - Do not display on screen or allow interaction (may still print)
  - __64__  - Do not allow user interaction (ignored for widget annotations)
  - __128__  - Do not allow deletion or position/size changes by the user
  - __256__  - Invert NoView for certain events (e.g. mouse-over display)
  - __512__  - Do not allow the annotation contents to be modified


- `overwrite` (bool) __Overwrite__ - Apply the flag change even when the F key already exists
...
- `remove_contents` (bool) __Remove Contents__ - Remove the Contents key. For widget annotations, also removes the TU key

- `remove_keys` (string) __Remove Keys__ - Enter other optional annotation dictionary keys to remove, separated by commas. Safe targets include descriptive or metadata entries (T, Subj, RC, M, CreationDate) and optional visual entries (Border, C, BS, CA, BE). Warning: do not remove required or structural keys (Type, Subtype, Rect, P, StructParent, NM, AP, AS, A, Dest, Popup, IRT) — doing so can corrupt annotations or make the PDF invalid

  - ____  - None
  - __T,Subj,RC__  - Descriptive text (T, Subj, RC)
  - __M,CreationDate__  - Dates (M, CreationDate)
  - __State,StateModel__  - Text annotation state (State, StateModel)
  - __Border,C,BS,CA__  - Visual style (Border, C, BS, CA)
...
- `flags` (flag) __Cleanup__ - Specify types of marked content to remove

  - __8__  - Remove Invalid MCID
  - __4__  - Remove Custom Content Mark
  - __1__  - Remove MCID
  - __2__  - Remove Artifact


- `content_mode` (string) __Fix conflicted MCIDs__ - Choose whether conflicting artifact and tagged content should remain tagged real content or become an artifact

  - __none__  - None
  - __tagged__  - Keep as tagged content
  - __artifact__  - Mark as artifact


- `sync_tag_names` (bool) __Sync content tag names__ - Rename marked content tag names to match the linked structure tag type (for example content mark H1 when the structure tag is P)
...
- `fill_color` (string) __Fill Color__ - Specify a new fill color: RGB with each component 0-255 (e.g. RGB(127,255,0)), or CMYK with each component 0-100 (percent-like, e.g. CMYK(25,84,50,100)). Leave empty to keep the current color unchanged

- `stroke_color` (string) __Stroke Color__ - Specify a new stroke color: RGB with each component 0-255 (e.g. RGB(127,255,0)), or CMYK with each component 0-100 (percent-like, e.g. CMYK(25,84,50,100)). Leave empty to keep the current color unchanged
...
- `index` (int) __Index__ - Specify an index of the substring. Negative value indicates the last character and oposite direction (-1 is the last character, -2 is the second last, etc.)
...
- `add_missing_unicode` (bool) __Fix ToUnicode Mappings__ - Rebuild the ToUnicode CMap for fonts with missing Unicode mappings for used character codes.
...
- `font_names` (font) __Fonts__ - Specify the fonts using a ECMAScript regular expression or define them by template font_update

- `charcode` (int) __Charcode Number__ - Specify the charcode for which to set the Unicode mapping

- `unicode_value` (string) __Unicode Value__ - Specify the Unicode value to set for the charcode

- `overwrite` (bool) __Overwrite__ - Replace existing Unicode mappings if they already exist
...
Merge neighbouring structure tags in reading order ($0 above $1) by moving children from one tag into another

#### params:

- `tag_names` (tag) __Tags__ - Regex: both neighbours must match. Template preset: pair rules in tag_neighbours (cross-page, Note chains). Must match Neighbours — regex with Direct neighbours; presets with the corresponding Neighbours type.

- `adjacent_type` (string) __Neighbours__ - How neighbouring tag pairs are found. Empty (Direct neighbours): next tag in the structure tree after Skip tags wrappers are ignored, and page content (text, images) between two tags separates them — use with regex Tags. A tag type: consecutive tags of that type in reading order — use with the matching Tags template preset. Skip tags always apply: matching wrappers (NonStruct, Div, …) are stepped over as if they were not in the tree.

  - ____  - Next tag in the structure tree only (default). Use with regex Tags (e.g. Same headings (H1)).
  - __Note__  - Pair consecutive Note tags in reading order and skip other tag types in between. Use with the Note chains Tags preset.
  - __L__  - Pair consecutive L(List) tags in reading order and skip other tag types in between.
  - __P__  - Pair consecutive P(Paragraph) tags in reading order and skip other tag types in between.
  - __Table__  - Pair consecutive Table tags in reading order and skip other tag types in between.


- `skip_tags` (string) __Skip tags__ - Regular expression of tag types that are stepped over when neighbours are searched. A matching tag is treated as if it was not in the structure tree, so tags wrapped in it are still paired as neighbours. Empty pairs only tags with the same direct parent.

  - __^(Document|Part|Art|Sect|Div|BlockQuote|Caption|TOC|TOCI|Index|NonStruct|Private)$__  - Step over all grouping tags: Document, Part, Art, Sect, Div, BlockQuote, Caption, TOC, TOCI, Index, NonStruct, Private (default).
  - __^(Document|NonStruct|Div)$__  - Step over NonStruct and Div wrappers, keep section and list containers as boundaries.
  - ____  - Do not step over any tag; only tags with the same direct parent are paired.


- `merge_target` (string) __Merge into__ - Move children into the first or second tag of the pair ($0 is above $1 in reading order)

  - __first__  - Move children into the upper tag in reading order; the lower tag is removed (default).
  - __second__  - Move children into the lower tag in reading order; the upper tag is removed.


- `tag_name` (string) __Rename to__ - Rename the surviving tag to a new type (empty keeps the current type)

  - ____  - Leave the surviving tag type unchanged after merge (default).
  - __P__  - Rename the surviving tag to P(Paragraph) after merge.
  - __L__  - Rename the surviving tag to L(List) after merge.
  - __Table__  - Rename the surviving tag to Table after merge.
...
```
{
    ...
    "params": [
        ...
        {
            "name": "skip_tags",
            "value": "^(Document|Part|Art|Sect|Div|BlockQuote|Caption|TOC|TOCI|Index|NonStruct|Private)$"
        },
        {
            "name": "merge_target",
            "value": "first"
...
- `alt_type` (int) __Alternative Description__ - Define a source for detecting the alternative text
  ...
  - __3__  - Use tag text
  - __4__  - Use tag text and Contents for annotations
