...
| `graphic_table_detect` | `1` | Master switch for path-based (graphic) table detection. 0 disables building tables from vector paths; 1 enables the graphic-table pipeline. |
| `graphic_table_detect_row` | `1` | If 0, y-coordinates from path lines and rectangle edges are not used to build row bands for path-based (graphic) tables; if 1, horizontal rules contribute row structure. |
| `graphic_table_detect_col` | `1` | If 0, x-coordinates from path lines and rectangle edges are not used to build column guides for path-based (graphic) tables; if 1, vertical rules contribute column structure. |
...
| `text_table_detect` | `1` | Master switch for text-based section tables (columns built from text/image chunks, not from vector grid paths). |
| `text_table_detect_row` | `1` | If 0, isolated text and images are not grouped into horizontal row bands for borderless (text-based) table hypotheses; if 1, row clustering runs. |
| `text_table_detect_col` | `1` | If 0, isolated elements are not clustered into vertical column bands for text-only table hypotheses; if 1, column alignment is evaluated. |
...
Number of low-level PDF objects (paths, text runs, images) merged into this layout element.
